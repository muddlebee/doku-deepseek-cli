# doku MCP Configuration Guide

doku supports MCP (Model Context Protocol), enabling AI assistants to connect with external tools and services such as GitHub, browsers, databases, and more.

## Overview

Once MCP is configured, doku can:
...
MCP tools are named in doku using the format `mcp__<service_name>__<tool_name>`, for example `mcp__github__search_code`.

## Configuring MCP Servers

Edit `~/.doku/settings.json` and add the `mcpServers` field:
...
| --------- | -------- | -------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `command` | string   | Yes      | Path or command of the MCP server executable (e.g., `npx`, `node`, `python`). When the command is `npx`, doku automatically prepends `-y` to the arguments. |
...
Allows doku to directly operate on GitHub repositories (search code, manage issues/PRs, read/write files, etc.):
...
Lets doku control a browser for screenshots, page interactions, etc.:
...
Enables doku to read and write files within a specified directory:
...
Below is a complete `~/.doku/settings.json` with both GitHub and Playwright MCP servers configured:
...
After configuration, start `doku` and type `/mcp` in the chat to view the status of all configured MCP servers and the list of tools each server provides.

Simply use the MCP tool name in your conversation to invoke it, for example:

```
Help me search for issues in the deepseek-cli repository on GitHub
```
...
3. Whether the terminal running `doku` has network access
...
On Windows, doku automatically adds shell support for `.cmd` commands. If your MCP command is a batch script, ensure the filename ends with `.cmd`.
