...
doku 使用 `settings.json` 设置文件进行持久化配置，支持两个层级的存放位置：
...
| `credentialProvider` | string    | 已由设置向导确认保存凭据的提供商                                     |
...
doku 内置免费可用的 Web Search 工具。如果需要自定义搜索逻辑，可将 `webSearchTool` 设为一个可执行脚本的完整路径：
...
> 当 `command` 为 `npx` 时，doku 会自动在参数前补充 `-y`。
