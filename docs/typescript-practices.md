...
- Separate pure domain and state-transition logic from persistence, transcript, UI, and provider integration. Facades should coordinate focused modules rather than implement domain rules directly.
...
- Use automated tests as the default validation for focused UI changes. Build the CLI and exercise the affected journey in a real PTY only when a change materially affects keyboard handling, focus or navigation, cancellation/resume, queued input, asynchronous status transitions, raw-mode lifecycle, or terminal reflow. Those behaviors are not represented reliably by piped stdin or non-TTY process capture.
- Use a PTY-capable terminal or runner, launch `node dist/cli.js`, send the actual key sequence a user would enter, and verify both the visible result and exit/cancellation behavior. Use one representative width for a focused interaction change; check 60/80/120 columns only when responsive layout or wrapping can change.
- Keep required PTY checks focused and deterministic. Use mocked providers or local fixtures unless the change specifically concerns a live provider. Follow the full cross-provider journeys in [`dogfooding.md`](dogfooding.md) only for major terminal workflows, release readiness, provider integration changes, or when explicitly requested.
